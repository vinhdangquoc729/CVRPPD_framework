...
class ClusterGASolverPD(Solver):
    """
    1) Gom khách thành các cụm nhỏ bằng k-means.
    ...
    def _co_locate_pd_pairs(self) -> None:
        """
        Với mỗi cặp (P,D), nếu P và D ở 2 cụm khác nhau:
        - Di chuyển node thuộc cụm có kích thước lớn hơn sang cụm có kích thước nhỏ hơn.
        - Cập nhật lại self.cust2cluster.
        """
        pairs = self._gather_pd_pairs()
        if not pairs or not self.clusters:
            return

        # map node -> cluster idx
        cust2cluster: Dict[int, int] = {}
        for c_idx, group in enumerate(self.clusters):
            for nid in group:
                cust2cluster[nid] = c_idx

        for p, d in pairs:
            cp = cust2cluster.get(p, None)
            cd = cust2cluster.get(d, None)
            if cp is None or cd is None or cp == cd:
                continue

            # chọn nguồn/đích theo size (đẩy từ cụm đông -> cụm ít)
            size_cp = len(self.clusters[cp])
            size_cd = len(self.clusters[cd])

            # tránh làm rỗng cụm nguồn (an toàn)
            if size_cp == 1 or size_cd == 1:
                # vẫn ưu tiên chuyển từ cụm đông hơn, nhưng nếu nguồn còn 1 thì chuyển chiều
                pass

            if size_cp > size_cd:
                # chuyển p sang cụm cd
                if p in self.clusters[cp] and size_cp > 1:
                    self.clusters[cp].remove(p)
                    self.clusters[cd].append(p)
                    cust2cluster[p] = cd
            else:
                # chuyển d sang cụm cp
                if d in self.clusters[cd] and size_cd > 1:
                    self.clusters[cd].remove(d)
                    self.clusters[cp].append(d)
                    cust2cluster[d] = cp

        # loại cụm rỗng nếu có (hiếm khi xảy ra với quy tắc trên)
        self.clusters = [g for g in self.clusters if g]

        # rebuild cust2cluster cho chắc
        self.cust2cluster = {}
        for c_idx, group in enumerate(self.clusters):
            for nid in group:
                self.cust2cluster[nid] = c_idx
    ...
    def _random_chromosome(self) -> Chromosome:
        ...
        for group in self.clusters:
            g = group[:]
            self.rng.shuffle(g)
            g = self._fix_pd_precedence_intra(g)  # đảm bảo pickup đứng trước delivery ngay trong cụm
    ...
    def _gather_pd_pairs(self) -> List[Tuple[int, int]]:
        """Lấy danh sách (pickup, delivery) từ problem.pd_pairs (nếu có)."""
        ...
        if isinstance(pd, dict):
            for p, tup in pd.items():
                if isinstance(tup, (tuple, list)) and len(tup) >= 1:
                    d = int(tup[0])
                    pairs.append((int(p), d))
        return pairs

    def _fix_pd_precedence_intra(self, seq: List[int]) -> List[int]:
        """Đảm bảo trong 1 cụm: vị trí(pickup) < vị trí(delivery) cho mọi cặp nằm trong seq."""
        pd_pairs = getattr(self.problem, "pd_pairs", {})
        pos = {x: i for i, x in enumerate(seq)}
        changed = True
        loops = 0
        while changed and loops < 4:  # vài vòng là đủ
            changed = False
            loops += 1
            for p, tup in pd_pairs.items():
                d = tup[0] if isinstance(tup, (tuple, list)) and tup else None
                if d is None: 
                    continue
                if p in pos and d in pos and pos[d] < pos[p]:
                    # hoán đổi vị trí để p trước d
                    i, j = pos[p], pos[d]
                    seq[i], seq[j] = seq[j], seq[i]
                    pos[seq[i]], pos[seq[j]] = i, j
                    changed = True
        return seq
    ...
    def _mutate(self, x: Chromosome) -> None:
        ...
        # pd-pair move mutation (thử 3 ứng viên xe và chọn tốt nhất)
        if self.rng.random() < 0.2:
            moved = self._mutate_move_pd_pair(x, try_k=3)
        
        # assignment: reassign cụm (nhẹ)
        m = max(1, nC // 20)
        ...
        for _ in range(self.rng.randrange(1, k + 1)):
            ...
            if len(seq) >= 3 and self.rng.random() < 0.5:
                ...
                seq[i], seq[j] = seq[j], seq[i]
            # giữ precedence sau khi xáo trộn
            x.intra_orders[c] = self._fix_pd_precedence_intra(seq)

        # cluster_order: swap + đảo đoạn
    ...
    def _mutate_move_pd_pair(self, x: Chromosome, try_k: int = 3) -> bool:
        """
        Đột biến: chuyển 1 cặp (P,D) sang route khác bằng cách đổi assignment
        của cụm chứa cặp đó (hoặc 2 cụm nếu P,D khác cụm).
        - try_k: số ứng viên xe thử để chọn phương án tốt nhất (nếu <=1 thì chọn ngẫu nhiên).
        Trả về True nếu có thay đổi; False nếu bỏ qua.
        """
        pairs = self._gather_pd_pairs()
        if not pairs or not self.vehicles or not self.clusters:
            return False

        # chọn 1 cặp PD ngẫu nhiên
        p, d = self.rng.choice(pairs)

        # tra cụm đang chứa p,d
        cp = self.cust2cluster.get(p, None)
        cd = self.cust2cluster.get(d, None)
        if cp is None or cd is None:
            return False

        affected_clusters = {cp}
        if cd != cp:
            affected_clusters.add(cd)

        # xe hiện tại (nếu 2 cụm khác nhau, ưu tiên lấy xe của cụm 'lớn' hơn)
        def cluster_size(cidx: int) -> int:
            return len(self.clusters[cidx]) if 0 <= cidx < len(self.clusters) else 0

        base_c = max(affected_clusters, key=cluster_size)
        cur_v = x.assignment[base_c]

        # ứng viên xe đích (khác cur_v)
        all_vidx = list(range(len(self.vehicles)))
        cand_vidx = [v for v in all_vidx if v != cur_v]
        if not cand_vidx:
            return False

        # nếu chỉ thử ngẫu nhiên
        if try_k <= 1:
            new_v = self.rng.choice(cand_vidx)
            for cidx in affected_clusters:
                x.assignment[cidx] = new_v
            # đảm bảo precedence bên trong cụm (optional, phòng xa)
            for cidx in affected_clusters:
                x.intra_orders[cidx] = self._fix_pd_precedence_intra(x.intra_orders[cidx])
            return True

        # thử một vài xe đích, giữ cost tốt nhất
        sampled = self.rng.sample(cand_vidx, min(try_k, len(cand_vidx)))
        best_cost = None
        best_v = None

        # snapshot nhanh để thử
        def try_with(v_idx: int) -> float:
            # clone mỏng của chromosome chỉ để đánh giá
            tmp = Chromosome(
                assignment=x.assignment[:],
                intra_orders=[seq[:] for seq in x.intra_orders],
                cluster_order=x.cluster_order[:]
            )
            for cidx in affected_clusters:
                tmp.assignment[cidx] = v_idx
                tmp.intra_orders[cidx] = self._fix_pd_precedence_intra(tmp.intra_orders[cidx])
            # decode & evaluate
            sol = self._decode(tmp)
            res = evaluator(self.problem, sol, return_details=False)
            return res[0] if isinstance(res, tuple) else float(res)

        for v_idx in sampled:
            cost = try_with(v_idx)
            if (best_cost is None) or (cost < best_cost):
                best_cost, best_v = cost, v_idx
